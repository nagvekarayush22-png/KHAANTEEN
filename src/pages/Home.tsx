  ...
  return (
    <div className="relative min-h-screen bg-white">
      ...
      <div className="fixed right-6 top-1/2 -translate-y-1/2 z-40 hidden xl:flex flex-col gap-2">
        {[
          ...
          <motion.button
            key={i}
            whileHover={{ x: -10 }}
            className="w-14 h-14 bg-white shadow-xl rounded-2xl flex flex-col items-center justify-center gap-1 group border border-gray-100 hover:border-brand-orange/30 transition-all"
          >
            <div className="text-gray-400 group-hover:text-brand-orange transition-colors">
              {item.icon}
            </div>
            <span className="text-[8px] font-black uppercase text-gray-300 group-hover:text-brand-orange whitespace-nowrap">
              {item.label}
            </span>
          </motion.button>
        ))}
      ...
      <section className="relative min-h-[85vh] lg:h-[90vh] overflow-hidden flex flex-col justify-center">
        {/* Campus Panorama - White theme compatible */}
        <div 
        ...
        <div className="relative w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-32 md:pb-48 z-10 text-center md:text-left">
          ...
          >
             <div className="bg-brand-orange text-white text-[9px] sm:text-[10px] font-black px-3 py-1 rounded-full w-fit mb-6 uppercase tracking-widest shadow-lg shadow-orange-500/20 mx-auto md:mx-0">
                KHAANTEEN Smart E-Canteen
             </div>
             <div className="flex flex-col mb-8 p-0">
                <div className="flex items-center justify-center md:justify-start gap-4 mb-4">
                  <Logo showText={false} className="scale-125 md:scale-150 origin-left" />
                  <div className="h-14 w-px bg-gray-900/20 hidden sm:block mx-4" />
                  <span className="hidden sm:block text-gray-900/50 text-[10px] font-black uppercase tracking-[0.4em] rotate-180 [writing-mode:vertical-lr]">EST 2024</span>
                </div>
                
                <h1 className="text-5xl md:text-8xl font-black text-black uppercase font-display mt-4 leading-tight">
                   SMART FOOD.<br />
                   <span className="text-brand-orange">DELIVERED.</span>
                </h1>
             </div>
             <p className="text-gray-600 text-base sm:text-lg md:text-xl font-medium mb-10 max-w-md mx-auto md:mx-0 leading-relaxed">
             ...
             <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center justify-center md:justify-start">
               <Link to="/menu" className="group px-8 py-4 bg-brand-orange text-white rounded-2xl font-black text-sm flex items-center justify-center gap-3 shadow-2xl shadow-orange-500/30 hover:bg-orange-600 transition-all">
                 <Utensils size={18} className="group-hover:rotate-12 transition-transform" />
                 Order Now
               </Link>
               <Link to="/menu" className="px-8 py-4 bg-white text-gray-900 border border-gray-200 rounded-2xl font-black text-sm flex items-center justify-center gap-3 hover:bg-gray-50 transition-all">
                 Explore Menu
                 <ArrowRight size={18} />
               </Link>
             </div>
                  ...
                  {categories.map((cat) => (
                    <button
                      ...
                      className={cn(
                        ...
                        activeCategory === cat ? "bg-brand-orange text-white shadow-lg shadow-orange-500/20 border-brand-orange" : "text-gray-400 hover:text-gray-600 hover:bg-gray-50"
                      )}
                    >
                      {cat}
                    </button>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-8">
                {filteredItems.map((item) => (
                  <motion.div 
                    key={item.id}
                    whileHover={{ y: -5 }}
                    className="bg-white rounded-2xl sm:rounded-[32px] p-4 sm:p-6 border border-gray-100 flex flex-col group min-w-0 h-full shadow-[0_10px_30px_rgba(0,0,0,0.08)]"
                  >
                    <div className="relative aspect-square mb-4 sm:mb-6 rounded-xl sm:rounded-[24px] overflow-hidden shadow-sm shrink-0">
                      ...
                      <div className="absolute top-2 sm:top-3 right-2 sm:right-3 bg-white/90 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg sm:rounded-xl flex items-center gap-1 shadow-sm">
                    ...
                    <div className="flex flex-col flex-1">
                      ...
                      <div className="flex items-center justify-between mt-auto pt-4 sm:pt-6 border-t border-gray-100 gap-2">
                        <span className="text-base sm:text-2xl font-display font-black text-gray-900">₹{item.price}</span>
                        <Link to="/menu" className="bg-brand-orange text-white p-2 sm:p-2.5 rounded-xl sm:rounded-2xl hover:scale-110 transition-transform shadow-md shadow-orange-500/20 shrink-0">
